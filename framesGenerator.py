...
dates_available = (last_date - first_date).days + 6

days_range = datetime.timedelta(days=dates_available)

starting_date = first_date - days_range
ending_date = first_date + days_range
...
for index in range(5,len(keys)):
    confirmed_variation_matrix = confirmed[keys[index]] - confirmed[keys[index - 1]]
    confirmed_increment_matrix = confirmed_variation_matrix / FRAMES_PER_DAY
    current_day_confirmed_matrix = confirmed[keys[index - 1]]

    confirmed_variation = confirmed_per_day[keys[index]] - confirmed_per_day[keys[index - 1]]
    confirmed_increment = confirmed_variation / FRAMES_PER_DAY
    current_day_confirmed = confirmed_per_day[keys[index - 1]]

    deaths_variation = deaths_per_day[keys[index]] - deaths_per_day[keys[index - 1]]
    deaths_increment = deaths_variation / FRAMES_PER_DAY
    current_day_deaths = deaths_per_day[keys[index - 1]]

    recovered_variation = recovered_per_day[keys[index]] - recovered_per_day[keys[index - 1]]
    recovered_increment = recovered_variation / FRAMES_PER_DAY
    current_day_recovered = recovered_per_day[keys[index - 1]]
    ...
    for frame in range(FRAMES_PER_DAY):
        ...
        dates_text = plot.text(dates_string_position, -85, dates_string , ha='center', va='bottom', size=4, color='white', family="monospace", in_layout=False)
        ...
        frame_number += 1
        dates_string_position -= 25.953 / FRAMES_PER_DAY
        current_day_confirmed_matrix = current_day_confirmed_matrix + confirmed_increment_matrix
